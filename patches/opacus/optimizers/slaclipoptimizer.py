...
class _SlaClipBase(DPOptimizer):
    ...
    def clip_and_accumulate(self):
        ...
        self._last_batch_size = int(B) if B is not None else 0
    ...
    def _compute_slack_indicator(self, C_t: float) -> Optional[torch.Tensor]:
        ...
        s_hat = slack_noisy_sum / (self._lambda_t * float(self._last_batch_size))
        return s_hat
    ...
    def add_noise(self):
        ...
        # Eq. (9): same-query Gaussian release in R^{d+K} (noise std = sigma*C_t/B)
        # Eq. (10)-(11): slack indicator normalization
        if self._slack_sum is None or self._last_batch_size <= 0:
            return
        if self._lambda_t <= 0:
            return
...
class SlaClipQOptimizer(_SlaClipBase):
    ...
    def _update_threshold(self, C_t: float, s_hat: torch.Tensor) -> float:
        ...
        return float(C_t * math.exp(self.eta * (self.gamma - s1)))
